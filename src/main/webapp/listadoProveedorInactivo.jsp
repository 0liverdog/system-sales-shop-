<%@ page import="java.util.List, Entidades.Proveedor, Dao.Dao" %>
...
.nav-item.active svg {
  stroke: var(--amber);
  filter: drop-shadow(0 0 8px rgba(245,166,35,.12));
}
...
<%
Dao dao = new Dao();
List<Proveedor> lista = dao.listarProveedorPorEstado(0);
%>
<aside class="sidebar">
...
<main class="main">
	...
	<h1 class="page-title">
      ...
      <span class="page-title-text">Proveedores Inactivos</span>
	</h1>
</div>
	...
	<div class="table-card">
		...
		<div class="empty-state-text">No hay proveedores activos</div>
		</div>
		</td>
		</tr>
		...
		<tr>
		    <td style="font-weight: 600; color: var(--text-3);"><%= p.getId_proveedor() %></td>
		    <td style="text-align: left; font-weight: 500;"><%= p.getNombre() %></td>
		    <td style="color: var(--amber); font-weight: 600;"><%= p.getRuc() %></td>
		    <td style="color: var(--text-2);"><%= p.getEstado() %></td>
		    <td>
		    	<span class="badge badge-inactive">Inactivo</span>
		    </td>
		    <td>
		    <a href="CambiarEstado?tabla=proveedor&campo=id_proveedor&id=<%=p.getId_proveedor()%>&estado=1"
		    ...
		    <td>
		    <a href="EliminarProveedor?id=<%=p.getId_proveedor() %>"
		    onclick="return confirm('¿Seguro que quieres eliminar este producto?')" class="btn btn-danger btn-sm">
		    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <a href="listadoProveedor.jsp" class="btn btn-success">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <polyline points="20 6 9 17 4 12"/>
      </svg>
